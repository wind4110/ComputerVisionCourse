...
def train(faces: list, energyRatio: float, avgFilePath: str,
          modelFilePath: str) -> tuple:
    """
    训练模型\n
    :param faces: 人脸数据
...
def reconstruct(face: np.ndarray, avg: np.ndarray, baseVecs: np.ndarray,
                N_PCS: int) -> np.ndarray:
    """
    将传入人脸变换到特征脸空间再重构\n
    :param face: 待重构人脸
    :param avg: 平均脸
    ...
    for i in range(0, N_PCS):
        vec = baseVecs[:, i]
        vec = np.resize(vec, (N_PIXEL, 1))
        reconFace = np.add(np.float64(reconFace),
                           np.float64(faceCoord[i] * vec))
...
def computeCoord(faceMat: np.ndarray, avg: np.ndarray, baseVecs: np.ndarray,
...
def findMostSimilar(face: np.ndarray, faces: list, avg: np.ndarray,
                    baseVecs: np.ndarray, N_PCS: int) -> np.ndarray:
    """
    在人脸库中寻找与给定人脸最相似的人脸\n
    :param face: 输入人脸
    :param faces: 人脸库
...
def identify(face: np.ndarray, faces: list, avg: np.ndarray,
             baseVecs: np.ndarray, N_PCS: int) -> None:
    """
    查找并显示最相似的人脸\n
    :param face: 待识别人脸
    :param faces: 人脸库
            ...
            for innerRoot, innerDirs, innerFiles in os.walk(outerDir):
                for file in innerFiles:
                    ...
                    face = cv2.cvtColor(face,
                                        cv2.COLOR_BGR2GRAY)  # 读入的人脸是三通道灰度图
                    faces.append(face)
    myFaces = []
...
def findMostSimilarObj(libFaceObjs: list, faceCoord: np.ndarray,
                       libFaceCoords: list, N_PCS: int) -> Face:
    """
...
def batchTest(N_PCS: int) -> float:
    ...
    for testFaceObj in testFaceObjs:
        testFaceCoord = computeCoord(testFaceObj.face, avg, baseVecs,
                                     baseVecs.shape[1])
        mostSimilar = findMostSimilarObj(trainFaceObjs, testFaceCoord,
                                         libFaceCoords, N_PCS)
        # 展示匹配结果
        # plt.subplot(2, 1, 1)
...
def main() -> None:
    ...
    avg, baseVecs = importModel("Open Models/avg-ORL.npy",
                                "Open Models/model-ORL-0.99.npy")
